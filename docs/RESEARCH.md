# RESEARCH â€” findings that shaped the design
...
| C: free space | **4.4 GB** â€” effectively full |
...
- `RunningMode.LIVE_STREAM` internally tracks between frames so palm detection does not re-run every frame â€” materially lower latency and CPU than re-detecting. Use it, not `IMAGE` mode in a loop.
- `GestureRecognizer` ships 7 canned gestures: ðŸ‘ ðŸ‘Ž âœŒï¸ â˜ï¸ âœŠ ðŸ‘‹ ðŸ¤Ÿ.
- Custom gestures: MediaPipe Model Maker trains a classification head on landmark embeddings and exports `.task`. Requires TensorFlow â€” heavy.
- **PyPI**: `mediapipe` is at **1.0.1**, and `pip index versions mediapipe` under Python 3.12 resolves it, so 3.12 wheels exist. MediaPipe supports **3.8â€“3.12 only**; 3.13+ requires building from source.

## Wake word

- **openWakeWord** â€” open source, free, unlimited, ships pretrained models including `hey_jarvis`, `alexa`, `hey_mycroft`. On Windows it installs `onnxruntime` (tflite-runtime has no modern Windows support). Custom models trainable via a Colab notebook in ~1 hour.
- **Porcupine** (Picovoice) â€” easier custom wake words but commercial licensing, quoted around $6K/yr for full SDK. Disqualified by the "free and unlimited" requirement.
- Benchmarks in the wild report openWakeWord matching or beating Porcupine on accuracy for equivalent training data.

## Speech to text

- **Vosk** â€” ~50 MB small English model, ~300 MB RAM at runtime, streaming with zero-latency partials, and crucially supports a **constrained grammar** (pass a JSON list of allowed phrases) which sharply improves accuracy for a fixed command vocabulary.
- **faster-whisper** â€” better raw accuracy (CTranslate2, 2Ã— faster than Whisper on CPU, int8 quantisation) but heavier and higher latency; better suited to confirmed segments than to snappy commands.
- Common production pattern is a hybrid: Vosk for low-latency partials, Whisper for confirmed segments, ~500â€“800 ms latency.
...
- **Volume**: `pycaw` on Windows (Core Audio `IAudioEndpointVolume`), AppleScript via `osascript` on macOS, `pactl`/`amixer` on Linux. No single cross-platform library is trustworthy â€” write a thin platform adapter.
...
- **Code signing**: an unsigned .exe triggers SmartScreen. EV certificates no longer bypass it (that behaviour was removed in 2024) â€” everything now builds reputation over time. Cheapest real option is Azure Trusted Signing at roughly $10/month; Sigstore does not produce Authenticode signatures Windows trusts. **Decision: ship unsigned, publish SHA-256 checksums, document the "More info â†’ Run anyway" step in the README.** Revisit if downloads justify $10/mo.
...
- Smaller webcam tools exist (Alea-AirCursor, InScroll, Zesture) â€” each covers one narrow slice (cursor, or scroll, or media).
