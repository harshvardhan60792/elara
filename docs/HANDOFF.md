...
You are picking up **Beckon**, a desktop app that lets someone control their computer with hand gestures, voice, and webcam presence — fully offline, free, no account, no cloud.
...
Every comparable project on GitHub is a 200-line "virtual mouse" script. The gap Beckon fills: it is an actual installable product — profiles, misfire prevention, a settings UI, an onboarding wizard, custom gesture training, plugins, and a signed-off performance budget.

## Where to start

1. `docs/PROGRESS.md` — the live checklist. The lowest unchecked box is your task.
2. `docs/PLAN.md` — every task's full spec: files to touch, what to implement, acceptance criteria.
3. `CLAUDE.md` — the hard rules (D-drive only, Python 3.12, dry-run, no attribution).
4. `docs/ARCHITECTURE.md` — how the pieces fit and why the threading model is what it is.
5. `docs/DECISIONS.md` — every choice already made and the reason. Do not re-litigate these; if you must reverse one, append a superseding ADR.
6. `docs/GESTURES.md` — the gesture vocabulary and the action catalogue.
7. `docs/TESTING.md` — how to verify without a camera, and what only a human can check.
